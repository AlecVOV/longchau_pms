...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const faqs = [
      ...
      },
      {
        ...
        answer: `<p>Yes! We can transfer most prescriptions from other pharmacies:</p>
             ...
             <p class="mt-3">Contact us at (028) 73023456 to start a transfer.</p>`
      },
      {
        id: 7,
        category: "prescriptions",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</li><li aria-current="page"><div class="flex items-center"><span class="mx-2 text-neutral-400">/</span><span class="text-neutral-700">FAQ</span></div></li></ol></nav><div class="text-center mb-12"><h1 class="text-3xl md:text-4xl font-bold text-neutral-900 mb-4">Frequently Asked Questions</h1><p class="text-lg text-neutral-600 max-w-2xl mx-auto"> Find answers to common questions about our pharmacy services, prescriptions, shipping, and more. </p></div><div class="max-w-2xl mx-auto mb-12"><div class="relative"><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Search FAQ..." class="w-full px-4 py-3 pl-12 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span class="i-iconify-ph-magnifying-glass text-neutral-400"></span></div></div></div><div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12"><!--[-->`);
      ssrRenderList(categories, (category) => {
        _push(`<button class="${ssrRenderClass([
          "p-4 rounded-lg border-2 transition-all duration-200 text-left",
          activeCategory.value === category.id ? "border-primary-500 bg-primary-50 text-primary-700" : "border-neutral-200 hover:border-neutral-300 text-neutral-700"
        ])}"><div class="flex items-center space-x-3"><span class="${ssrRenderClass(category.icon + " text-xl")}"></span><div><div class="font-semibold text-sm">${ssrInterpolate(category.name)}</div><div class="text-xs text-neutral-500">${ssrInterpolate(category.count)} questions</div></div></div></button>`);
      });
      _push(`<!--]--></div><div class="space-y-4"><!--[-->`);
      ssrRenderList(filteredFAQs.value, (faq) => {
        _push(`<div class="bg-white rounded-lg shadow-sm border border-neutral-200"><button class="w-full px-6 py-4 text-left flex items-center justify-between hover:bg-neutral-50 transition-colors"><h3 class="font-semibold text-neutral-900 pr-4">${ssrInterpolate(faq.question)}</h3><span class="${ssrRenderClass([
          "i-iconify-ph-caret-down transition-transform duration-200",
          openFAQs.value.includes(faq.id) ? "rotate-180" : ""
        ])}"></span></button>`);
        if (openFAQs.value.includes(faq.id)) {
          _push(`<div class="px-6 pb-4 text-neutral-600 border-t border-neutral-100"><div class="pt-4">${faq.answer ?? ""}</div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
      ...
      if (filteredFAQs.value.length === 0) {
        _push(`<div class="text-center py-12"><div class="w-16 h-16 bg-neutral-100 rounded-full flex items-center justify-center mx-auto mb-4"><span class="i-iconify-ph-question text-2xl text-neutral-400"></span></div><h3 class="text-lg font-semibold text-neutral-900 mb-2">No questions found</h3><p class="text-neutral-600 mb-6">Try adjusting your search terms or browse by category.</p><button class="bg-primary-600 text-white px-6 py-2 rounded-lg hover:bg-primary-700 transition-colors"> Clear Search </button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-8 mt-12"><div class="text-center"><h2 class="text-2xl font-bold text-neutral-900 mb-4">Still have questions?</h2><p class="text-neutral-600 mb-6"> Our customer service team is here to help you with any questions not covered in our FAQ. </p><div class="flex flex-col sm:flex-row gap-4 justify-center">`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/contact",
      ...
      _push(`<a href="tel:(028) 73023456" class="bg-white text-primary-600 border border-primary-600 px-6 py-3 rounded-lg hover:bg-primary-50 transition-colors font-medium"> Call (028) 73023456 </a></div></div></div></div></div>`);
    };
  }
};
...
//# sourceMappingURL=faq-CFChIukT.js.map
